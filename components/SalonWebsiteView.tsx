      ...
      <div className="max-w-7xl mx-auto px-4 xl:px-8 space-y-24 py-16 pb-32 md:pb-16">
          ...
          <section id="contact" className="bg-white dark:bg-slate-800 rounded-[2.5rem] p-8 md:p-12 border border-gray-200 dark:border-slate-700 shadow-sm scroll-mt-24">
             <div className="grid md:grid-cols-2 gap-12 items-center">
                <div className="space-y-8">
                   ...
                   <div className="space-y-6">
                      ...
                      <div className="flex items-start gap-4">
                         ...
                         <div>
                            <p className="text-sm text-gray-500 font-medium mb-1">{language === 'tr' ? 'Çalışma Saatleri' : 'Working Hours'}</p>
                            <div className="space-y-1">
                               <p className="font-bold text-gray-900 dark:text-white">{language === 'tr' ? 'Pazartesi - Cumartesi: 09:00 - 20:00' : 'Mon - Sat: 09:00 - 20:00'}</p>
                               <p className="font-semibold text-gray-500 dark:text-gray-400">{language === 'tr' ? 'Pazar: Kapalı' : 'Sunday: Closed'}</p>
                            </div>
                         </div>
                      </div>
                   </div>
                ...
                {/* Map/Location Card */}
                <div className="bg-slate-100 dark:bg-slate-900 rounded-3xl h-full min-h-[300px] border border-gray-200 dark:border-slate-700 overflow-hidden relative flex flex-col items-center justify-center text-center group">
                    ...
                    <div className="relative z-10 p-8 flex flex-col items-center justify-center h-full w-full bg-gradient-to-t from-slate-100 via-white/80 to-transparent dark:from-slate-900 dark:via-slate-800/80">
                        <div className="w-16 h-16 bg-white dark:bg-slate-800 rounded-full flex items-center justify-center text-red-500 mb-4 shadow-lg border border-gray-100 dark:border-slate-700">
                        ...
                        <h3 className="text-xl font-bold mb-2 text-gray-900 dark:text-white">{displayName}</h3>
                        <p className="text-sm font-medium text-gray-600 dark:text-gray-400 mb-6 max-w-[200px]">{businessProfile?.district || 'Merkez'}, {businessProfile?.city || 'Şehir'}</p>
                        
                        <div className="flex flex-col w-full max-w-xs gap-3">
                           {directionsUrl && (
                               <a href={directionsUrl} target="_blank" rel="noopener noreferrer" className="w-full bg-white dark:bg-slate-800 text-gray-900 dark:text-white py-3 rounded-xl font-bold shadow-sm hover:shadow-md transition-all border border-gray-200 dark:border-slate-600 flex items-center justify-center gap-2">
                                  <svg className="w-5 h-5 text-accent" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m0 0L9 7"/></svg>
                                  {language === 'tr' ? 'Haritada Aç' : 'Get Directions'}
                               </a>
                           )}
                           <button onClick={onStartBooking} className="w-full bg-accent text-white py-3 rounded-xl font-bold shadow-md hover:bg-blue-600 transition-all border border-transparent flex items-center justify-center gap-2">
                              {language === 'tr' ? 'Randevu Al' : 'Book Now'}
                           </button>
                        </div>
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-12">
               ...
               <div>
                  ...
                  <ul className="space-y-3 font-medium text-slate-400">
                     ...
                     <li><button onClick={() => window.location.href = `#/ai-visualizer?tenantId=${tenant?.id}`} className="hover:text-violet-400 transition-colors flex items-center gap-1.5"><span className="text-lg">🪄</span> {language === 'tr' ? 'AI Stil Asistanı' : 'AI Style'}</button></li>
