    ...
    <div className="w-full max-w-6xl mx-auto bg-slate-50 dark:bg-slate-900 min-h-screen relative shadow-2xl xl:rounded-[2rem] overflow-hidden xl:border-4 border-gray-100 dark:border-slate-800 pb-20 sm:pb-8">
      ...
      <div className="sticky top-0 bg-white/90 dark:bg-slate-800/90 backdrop-blur-md z-40 border-b border-gray-100 dark:border-slate-700 px-4 py-3 flex items-center justify-between">
         ...
         <div className="flex items-center gap-2 sm:gap-3">
            ...
            <button 
                ...
                className="w-10 h-10 flex items-center justify-center bg-violet-100 dark:bg-violet-900/40 text-violet-700 dark:text-violet-300 rounded-xl hover:bg-violet-200 dark:hover:bg-violet-900/60 transition-colors shadow-sm shrink-0" 
                title={language === 'tr' ? 'AI Stil Asistanı' : 'AI Style Assistant'}
            >
               <span className="text-xl">🪄</span>
            </button>
            <button onClick={onStartBooking} className="px-4 sm:px-5 py-2 bg-accent text-white rounded-xl font-bold text-sm shadow-sm hover:shadow-md hover:bg-blue-600 transition truncate max-w-[140px]">
      ...
      <div className="bg-white dark:bg-slate-800 overflow-hidden shadow-sm">
        ...
        >
          {coverImages.length > 0 ? (
              ...
              <div className="flex items-center justify-center h-full text-gray-400 text-sm">
                 {translations[language].salon.no_cover}
              </div>
          )}
        </div>
        ...
        <div className="px-6 md:px-12 pb-8 text-center max-w-4xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row justify-center items-stretch sm:items-center gap-3 mt-8">
             <button onClick={onStartBooking} className="flex-1 sm:flex-none w-full sm:w-auto min-w-[200px] bg-accent text-white py-4 px-8 rounded-2xl font-bold md:text-lg shadow-sm hover:shadow-lg hover:-translate-y-0.5 hover:bg-blue-600 transition-all text-center">
                {translations[language].salon.book_now}
             </button>
             <button onClick={() => window.location.href = `#/ai-visualizer?tenantId=${tenant?.id}`} className="flex-1 sm:flex-none w-full sm:w-auto min-w-[200px] bg-gradient-to-r from-violet-600 to-fuchsia-600 text-white py-4 px-8 rounded-2xl font-bold md:text-lg shadow-sm hover:shadow-lg hover:-translate-y-0.5 hover:opacity-90 transition-all flex items-center justify-center gap-2">
                <span>🪄</span> {language === 'tr' ? 'AI Stil Asistanı' : 'AI Style Assistant'}
             </button>
             {businessProfile?.whatsapp_number && (
                 <a href={`https://wa.me/${businessProfile.whatsapp_number.replace(/[^0-9]/g, '')}`} target="_blank" rel="noopener noreferrer" className="flex-1 sm:flex-none sm:w-auto min-w-[60px] bg-[#25D366] text-white py-4 px-6 rounded-2xl font-bold shadow-sm hover:shadow-lg hover:-translate-y-0.5 hover:bg-green-600 transition-all flex items-center justify-center gap-2" title="WhatsApp">
      ...
      <div className="grid lg:grid-cols-12 gap-8 lg:gap-12 px-6 md:px-12 mt-8 md:mt-12">
        <div className="lg:col-span-8 space-y-12">
          ...
          <div className="bg-gradient-to-r from-violet-50 to-amber-50 dark:from-violet-900/10 dark:to-amber-900/10 border border-violet-100 dark:border-violet-500/20 rounded-3xl p-6 md:p-8 flex flex-col sm:flex-row items-center justify-between gap-6 shadow-sm">
             <div>
                <h3 className="text-2xl font-bold text-violet-900 dark:text-violet-300 flex items-center gap-3">
                   <span className="text-3xl">🪄</span> {language === 'tr' ? 'Ne yaptıracağınıza karar veremediniz mi?' : 'Not sure what to book?'}
                </h3>
                <p className="text-base text-violet-700/80 dark:text-violet-400/80 mt-2">
                   {language === 'tr' ? 'Mevcut saç/stil fotoğrafınızı yükleyin, AI Stil Asistanı size en uygun işlemi ve uzmanı önersin.' : 'Upload your photo and let the AI Style Assistant recommend the best service for you.'}
                </p>
             </div>
             <div className="flex flex-col gap-3 shrink-0 w-full sm:w-auto">
                <button 
                  onClick={() => window.location.href = `#/ai-visualizer?tenantId=${tenant?.id}`}
                  className="w-full sm:w-auto text-center whitespace-nowrap bg-violet-600 hover:bg-violet-700 text-white font-bold py-3 px-6 rounded-xl transition-colors shadow-md shadow-violet-500/20"
                >
          ...
          {servicesList.length > 0 && (
            <div>
              <div className="flex items-center justify-between mb-6">
                 <h3 className="text-2xl font-bold text-gray-900 dark:text-white">{translations[language].salon.services}</h3>
                 <span className="text-sm font-medium text-accent bg-blue-50 dark:bg-slate-800 px-3 py-1 rounded-full">{translations[language].salon.services_count.replace('{count}', servicesList.length.toString())}</span>
              </div>
          ...
          {staffList.length > 0 && (
            <div>
              <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">{translations[language].salon.our_team}</h3>
              <div className="grid grid-cols-2 lg:grid-cols-3 gap-6">
                 {staffList.map((staff) => (
                    <div key={staff.id} className="bg-white dark:bg-slate-800 rounded-3xl p-6 border border-gray-100 dark:border-slate-700 text-center shadow-sm relative overflow-hidden flex flex-col items-center">
                        ...
                        {staff.image ? (
                           <img src={staff.image} alt={staff.name} className="w-24 h-24 mx-auto rounded-full object-cover mb-4 shadow-sm border-4 border-gray-50 dark:border-slate-700" referrerPolicy="no-referrer" />
                        ) : (
                        ...
                        <p className="text-sm text-gray-500 mt-1 line-clamp-1">{staff.title}</p>
                    </div>
                 ))}
              </div>
            </div>
        ...
        <div className="lg:col-span-4 mt-8 lg:mt-0">
           <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 md:p-8 shadow-sm border border-gray-100 dark:border-slate-700 lg:sticky lg:top-24">
              <div className="text-center mb-8">
                 ...
                 <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">{translations[language].salon.book_now}ın</h3>
                 <p className="text-sm text-gray-500">{translations[language].salon.book_now_desc}</p>
              </div>
              <button onClick={onStartBooking} className="w-full bg-accent text-white py-4 rounded-2xl font-bold text-lg shadow-md hover:shadow-lg hover:-translate-y-0.5 hover:bg-blue-600 transition-all mb-8 hidden sm:block">
                 {translations[language].salon.start_booking}
              </button>

              <div className="space-y-6 pt-8 border-t border-gray-100 dark:border-slate-700">
                  <h4 className="font-bold text-gray-900 dark:text-white text-sm uppercase tracking-wider text-center">{translations[language].salon.contact_info}</h4>
                  ...
                  {businessProfile?.phone && (
                      <div className="flex items-center gap-4 bg-gray-50 dark:bg-slate-700/50 p-4 rounded-2xl border border-gray-100 dark:border-slate-600">
                          ...
                          <div>
                              <p className="text-xs text-gray-500 mb-1">{translations[language].salon.phone}</p>
                  ...
                  {businessProfile?.address && (
                      <div className="flex items-start gap-4 bg-gray-50 dark:bg-slate-700/50 p-4 rounded-2xl border border-gray-100 dark:border-slate-600">
                          ...
                          <div>
                              <p className="text-xs text-gray-500 mb-1">{translations[language].salon.address}</p>
                              ...
                              {directionsUrl && (
                                 <a href={directionsUrl} target="_blank" rel="noopener noreferrer" className="text-accent text-sm font-bold hover:underline inline-flex items-center gap-1 mt-2">
                                     {translations[language].salon.open_in_maps}
                  ...
                  {(businessProfile?.instagram_url || businessProfile?.website_url) && (
                      <div className="flex items-center justify-center gap-4 pt-4">
                         ...
                         {businessProfile.website_url && (
                             <a href={businessProfile.website_url} target="_blank" rel="noopener noreferrer" className="w-12 h-12 rounded-full bg-white dark:bg-slate-700 shadow-sm border border-gray-100 dark:border-slate-600 flex items-center justify-center text-gray-500 hover:text-blue-600 hover:border-blue-200 hover:bg-blue-50 transition-all">
