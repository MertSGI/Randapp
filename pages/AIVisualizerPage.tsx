...
const AIVisualizerPage: React.FC = () => {
  ...
  const handleConsultation = async () => {
    ...
    const aiConfig = settingsStr ? JSON.parse(settingsStr) : { enableImageGeneration: true };

    const result = await GeminiService.generateFullConsultation(prompt, uploadedImage || undefined, language);
    
    if (result) {
        setAnalysisResult(result.text);
        if (aiConfig.enableImageGeneration && result.image) {
  ...
  const t = {
    en: {
      ...
      subtitle: "Upload your photo and describe what you want. Our AI will analyze your hair and suggest the best styles and services.",
      uploadTitle: "1. Upload Your Photo",
      uploadDesc: "Your photo is only used to generate recommendations. Please do not upload images containing sensitive personal information.",
    ...
    tr: {
      ...
      uploadTitle: "1. Fotoğrafınızı Yükleyin",
      uploadDesc: "Fotoğrafınız yalnızca öneri oluşturmak için kullanılır. Lütfen hassas kişisel bilgi içeren görseller yüklemeyin.",
            ...
            {(analysisResult || generatedImage) && (
               <div className="mt-8 pt-8 border-t border-gray-200 dark:border-slate-700">
                  ...
                  <div className="flex flex-col md:flex-row gap-8">
                     ...
                     {analysisResult && (
                         <div className={`w-full ${generatedImage ? 'md:w-1/2' : ''} prose prose-slate dark:prose-invert text-sm`}>
